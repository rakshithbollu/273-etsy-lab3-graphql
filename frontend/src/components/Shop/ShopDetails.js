...
const ShopDetails = ({ history }) => {
  ...
  const { loading, shopdetails, usershopdetails, shopsalesrevenue } =
    useSelector((state) => state.shopdetail);
  ...
  const uploadImage = async (e) => {
    ...
    // console.log('image',image);
    await axios
      .post("https://api.cloudinary.com/v1_1/dj3in4dua/image/upload", formData)
      .then((res) => {
  ...
  useEffect(() => {
    if (shopimage) {
      dispatch(saveShopImage(shopimage, email))
        .then(() => dispatch(loadUser()))
        .then(() => dispatch(getShopDetails(shopname)));
    }
  }, [shopimage]);
  ...
  const updateProfileSubmit = (e) => {
    e.preventDefault();
    if (newcategory) {
      dispatch(insertCategory(shopname, newcategory))
        .then(() =>
          dispatch(
            createProduct(
              productname,
        ...
        .then(() => dispatch(loadUser()))
        .then(() => dispatch(getShopDetails(shopname)))
        .then(() => dispatch(getCategory(shopname)));
      setShow(false);
    } else {
      dispatch(
        createProduct(
          ...
          <>
            <div class="container mt-4 mb-4 p-3 d-flex justify-content-center">
              <div class="card p-4">
                <div class=" image d-flex flex-column justify-content-center align-items-center">
                  ...
                  <span class="name mt-3">shopname: {user[0].shopname}</span>{" "}
                  <span class="idd">owner: {usershopdetails[0].name}</span>
                  <span class="idd">{usershopdetails[0].email}</span>
            ...
            <div className="profileContainer">
              <div>
                ...
                >
                  {shopname === user[0].shopname ? (
                    <h6>Total Sales Income from my shop:</h6>
                  ) : (
                    ""
                  )}
                  {shopsalesrevenue && shopname === user[0].shopname ? (
                    <h6>{shopsalesrevenue}</h6>
                  ) : (
                    ""
                  )}
                </div>
        ...
        <Modal show={showModal} onHide={handleClose}>
          ...
          <Modal.Body>
            <Form>
              ...
              <Form.Group className="mb-3">
                <Form.Select
                  aria-label="Select the Currency type"
                  value={currency}
                  name="currency"
                ...
                >
                  {categories &&
                    categories.map((categoryname) => (
                      <option value={categoryname.category}>
